...
## `Network`
`Network`中包含了`Graph`中的所有与节点相关的方法，还增加了**操作边以及操作顶点与边的关系**的方法，例如：

+ `outEdges(node)` ： 获取`node`的出度边

+ ` incidentNodes(edge)` ：获取边`edge`的顶点对

+ `edgesConnecting(nodeU, nodeV)` ：获取`nodeU`和`nodeV`的直连边
