 ...
 * {@link #threadAssertTrue}, {@link #threadAssertEquals}, or {@link #threadAssertNull}, (not {@code fail},
 * {@code assertTrue}, etc.) It is OK (but not particularly recommended) for other code to use these forms too. Only the
 * most typically used JUnit assertion methods are defined this way, but enough to live with.</li>
 * <li>If you override {@link #setUp} or {@link #tearDown}, make sure to invoke {@code super.setUp} and
 * {@code super.tearDown} within them. These methods are used to clear and check for thread assertion failures.</li>
 * <li>All delays and timeouts must use one of the constants {@code
 * SHORT_DELAY_MS}, {@code SMALL_DELAY_MS}, {@code MEDIUM_DELAY_MS}, {@code LONG_DELAY_MS}. The idea here is that a
 * SHORT is always discriminable from zero time, and always allows enough time for the small amounts of computation
 * (creating a thread, calling a few methods, etc) needed to reach a timeout point. Similarly, a SMALL is always
 * discriminable as larger than SHORT and smaller than MEDIUM. And so on. These constants are set to conservative
 * values, but even so, if there is ever any doubt, they can all be increased in one spot to rerun tests on slower
 * platforms.</li>
 ...
 * <b>Other notes</b>
 * <ul>
 * <li>Usually, there is one testcase method per JSR166 method covering "normal" operation, and then as many
 * exception-testing methods as there are exceptions the method can throw. Sometimes there are multiple tests per JSR166
 * method when the different "normal" behaviors differ significantly. And sometimes testcases cover multiple methods
 * when they cannot be tested in isolation.</li>
 * <li>The documentation style for testcases is to provide as javadoc a simple sentence or two describing the property
 * that the testcase method purports to test. The javadocs do not say anything about how the property is tested. To find
 * out, read the code.</li>
 ...
 * <li>The test classes currently do not declare inclusion in any particular package to simplify things for people
 * integrating them in TCK test suites.</li>
 * <li>As a convenience, the {@code main} of this class (JSR166TestCase) runs all JSR166 unit tests.</li>
 * </ul>
 */
public class JSR166TestCase extends TestCase {
	...
	/**
	 * Extra checks that get done for all test cases. Triggers test case failure if any thread assertions have failed,
	 * by rethrowing, in the test harness thread, any exception recorded earlier by threadRecordFailure. Triggers test
	 * case failure if interrupt status is set in the main thread.
	 */
	...
	/**
	 * Just like fail(reason), but additionally recording (using threadRecordFailure) any AssertionFailedError thrown,
	...
	/**
	 * Just like assertFalse(b), but additionally recording (using threadRecordFailure) any AssertionFailedError thrown,
	 * so that the current testcase will fail.
	 */
	...
	/**
	 * Just like assertNull(x), but additionally recording (using threadRecordFailure) any AssertionFailedError thrown,
	 * so that the current testcase will fail.
	 */
	...
	/**
	 * Just like assertEquals(x, y), but additionally recording (using threadRecordFailure) any AssertionFailedError
	 * thrown, so that the current testcase will fail.
	 */
	...
	/**
	 * Just like assertSame(x, y), but additionally recording (using threadRecordFailure) any AssertionFailedError
	 * thrown, so that the current testcase will fail.
	 */
	...
	/**
	 * Checks that the threads do not terminate within the default millisecond delay of {@code timeoutMillis()}.
	 */
	...
	/**
	 * Runs Runnable r with a security policy that permits precisely the specified permissions. If there is no current
	 * security manager, the runnable is run twice, both with and without a security manager. We require that any
	 * security manager permit getPolicy/setPolicy.
	 */
	...
	/**
	 * Waits up to LONG_DELAY_MS for the given thread to enter a wait state: BLOCKED, WAITING, or TIMED_WAITING.
	 */
