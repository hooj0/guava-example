...
/**
 * A test utility that verifies that your methods and constructors throw {@link NullPointerException} or
 * {@link UnsupportedOperationException} whenever null is passed to a parameter that isn't annotated with
 ...
 * <p>
 * The tested methods and constructors are invoked -- each time with one parameter being null and the rest not null --
 * and the test fails if no expected exception is thrown. {@code
 * NullPointerTester} uses best effort to pick non-null default values for many common JDK and Guava types, and also for
 * interfaces and public classes that have public parameter-less constructors. When the non-null default value for a
 * particular parameter type cannot be provided by {@code
 * NullPointerTester}, the caller can provide a custom non-null default value for the parameter type via
 * {@link #setDefault}.
 *
...
public final class NullPointerTester {
	...
	/**
	 * Runs {@link #testMethod} on every public static method of class {@code c}, including those "inherited" from
	 ...
	 * {@code minimalVisibility}, including those inherited from superclasses of the same package.
	 */
	 ...
	 * whenever <i>any</i> of its non-nullable parameters are null.
	 *
	 * @param instance
	...
	/**
	 * Verifies that {@code method} produces a {@link NullPointerException} or {@link UnsupportedOperationException}
	 * when the parameter in position {@code paramIndex} is null. If this parameter is marked nullable, this method does
	 * nothing.
	 *
	 * @param instance
	 *            the instance to invoke {@code method} on, or null if {@code method} is static
	 ...
	 * the parameter in position {@code paramIndex} is null. If this parameter is marked nullable, this method does
	 * nothing.
	 */
	 ...
	 * <p>
	 * The documentation for {@link Object#equals} says it should accept null, so don't require an explicit
	 * {@code @NullableDecl} annotation (see <a href="https://github.com/google/guava/issues/1819">#1819</a>).
	 * <p>
	 * It is not necessary to consider visibility, return type, or type parameter declarations. The declaration of a
	 * method with the same name and formal parameters as {@link Object#equals} that is not public and
	 * boolean-returning, or that declares any type parameters, would be rejected at compile-time.
	 */
